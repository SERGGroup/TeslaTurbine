...
figure
subplot(2,1,1)
plot([1:n],DATAbarH([1:n],2),'r*',[1:n],DATAbarS([1:n],2),'go',[1:n],EXP([1:n],2),'b-s')
xlabel('cases')  % should be updated to different parameters
ylabel('power (W)')
legend('Hmg. FLow Model','Sep. FLow Model','Exp data')
grid on
subplot(2,1,2)
plot([1:n],DATAbarH([1:n],1),'r*',[1:n],DATAbarS([1:n],1),'go',[1:n],EXP([1:n],1),'b-s')
xlabel('cases')  % should be updated to different parameters
ylabel('\eta')
legend('Hmg. FLow Model','Sep. FLow Model','Exp data')
grid on
...
BetaH=PinH./PoutH;
BetaS=PinS./PoutS;
plot(BetaH,DATAbarH([1:n],1),'bsq',BetaS,DATAbarS([1:n],1),'r*')
xlabel('\beta')
ylabel('Power (W)')
legend('Homo','Sep')
figure
...
plot(f1,'r')
plot(f2,'g')
legend('Exp data','Hmg. FLow Model','Sep. FLow Model')
...
bar([err1,err2]*100')
legend('Homogeneous model','Separate flow')
...
plot(DATAbarH(R,3),DATAbarH(R,1),'y-o',...
    DATAbarS(R,3),DATAbarS(R,1),'g-o',...
    EXP(R,3),EXP(R,1),'b-o')
i=19; j=23;R=[i:j]; % range
plot(DATAbarH(R,3),DATAbarH(R,1),'r--d',...
    DATAbarS(R,3),DATAbarS(R,1),'g--d',...
    EXP(R,3),EXP(R,1),'b--d')
i=24; j=26; R=[i:j]; % range
plot(DATAbarH(R,3),DATAbarH(R,1),'c:s',...
    DATAbarS(R,3),DATAbarS(R,1),'m:s',...
    EXP(R,3),EXP(R,1),'k:s')
legend( 'Hom. RPM3000','Sep. RPM3000','exp. fit RPM3000',...
        'Hom. RPM3500','Sep. RPM3500','exp. fit RPM3500',...
        'Hom. RPM4000','Sep. RPM4000','exp. fit RPM4000',...
        'Hom. RPM4500','Sep. RPM4500','exp. fit RPM4500')
