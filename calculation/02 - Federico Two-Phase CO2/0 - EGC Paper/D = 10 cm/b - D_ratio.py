...
D_ratio_ref = 3  # [m]
D_ratio_list = np.linspace(0.5, 1.2, n_setpoints) * D_ratio_ref

b_channel_ref = 0.0001  # [m]
b_channel_list = np.linspace(0.5, 1.5, n_setpoints) * b_channel_ref

D_ratio, b = np.meshgrid(D_ratio_list, b_channel_list)
...
Eta = np.empty(len(D_ratio_list) * len(b_channel_list))
Power = np.empty(len(D_ratio_list) * len(b_channel_list))

Eta_arr = np.empty(len(D_ratio_list) * len(b_channel_list))
Power_arr = np.empty(len(D_ratio_list) * len(b_channel_list))
RPM_arr = np.empty(len(D_ratio_list) * len(b_channel_list))

output_array = np.empty((len(range(n_setpoints)) * len(range(n_setpoints)), 13))

Eta_max_arr = np.empty(len(D_ratio_list))
D_max_arr = np.empty(len(D_ratio_list))
b_max_arr = np.empty(len(D_ratio_list))
...
for i in tqdm(range(n_setpoints)):
    ...
    for j in tqdm(range(n_setpoints)):
        ...
        curr_geometry.rotor.d_ratio = D_ratio[i,j]  # [m]
...
Eta_res = Eta_arr.reshape(D_ratio.shape)
Power_res = Power_arr.reshape(D_ratio.shape)
RPM_res = RPM_arr.reshape(D_ratio.shape)


# %%------------   PLOT RESULTS                            ----------------------------------------------------------> #
method = 'cubic'

res1 = (output_array[:, 1]).reshape(D_ratio.shape)
res2 = output_array[:, 5].reshape(D_ratio.shape)
res3 = ((output_array[:, 3] * output_array[:, 6]) / (output_array[:, 12] * 100000)).reshape(D_ratio.shape)
res4 = (output_array[:, 7] / 1000000).reshape(D_ratio.shape)

def interp_with_conditional_threshold(x, y, z, xi, yi, z_threshold=None, y_threshold=None, method='cubic'):
    z = z.flatten()
    x = x.flatten()
    y = y.flatten()
    mask = np.isfinite(z)

    if z_threshold is not None and y_threshold is not None:
        # Applica filtro su z solo se y > y_threshold
        mask &= (y <= y_threshold) | (z >= z_threshold)

    return griddata((x[mask], y[mask]), z[mask], (xi, yi), method=method)

def interp_with_conditional_threshold2(x, y, z, xi, yi, z_threshold=None, y_threshold=None, method='cubic'):
    z = z.flatten()
    x = x.flatten()
    y = y.flatten()
    mask = np.isfinite(z)
    ...
    return griddata((x[mask], y[mask]), z[mask], (xi, yi), method=method)
def interp_with_threshold(x, y, z, xi, yi, threshold=None, method='cubic'):
    z = z.flatten()
    mask = np.isfinite(z)
...
res1_interp = interp_with_threshold(x, y, res1, xi, yi, threshold=None, method=method)
res2_interp = interp_with_conditional_threshold(x, y, res2, xi, yi, z_threshold=None, method=method, y_threshold=None)
res3_interp = interp_with_conditional_threshold(x, y, res3, xi, yi, z_threshold=505, method=method, y_threshold=0.00011)
res4_interp = interp_with_conditional_threshold2(x, y, res4, xi, yi, z_threshold=11.4, method=method, y_threshold=0.00011)

# Ridisegna i grafici con i dati interpolati
fig, axs = plt.subplots(2, 2, figsize=(12, 8), constrained_layout=True)

ETA = axs[0, 0].contourf(xi, yi, res1_interp, 14, cmap='viridis')
MFR = axs[0, 1].contourf(xi, yi, res2_interp, 10, cmap='viridis')
SPEC_POWER = axs[1, 0].contourf(xi, yi, res3_interp, 10, cmap='viridis')
N_PACKS = axs[1, 1].contourf(xi, yi, res4_interp, 10, cmap='viridis')

font_axis = 14
font_title = 16

axs[0, 0].set_xlabel('Disk Gap [m]', fontsize=font_axis)
axs[0, 0].set_ylabel('Diameter Ratio [-]', fontsize=font_axis)
axs[0, 0].set_title('Efficiency [-]', fontsize=font_title)

axs[0, 1].set_xlabel('Disk Gap [m]', fontsize=font_axis)
axs[0, 1].set_ylabel('Diameter Ratio [-]', fontsize=font_axis)
axs[0, 1].set_title('Mass Flow Rate per Channel [kg]', fontsize=font_title)

axs[1, 0].set_xlabel('Disk Gap [m]', fontsize=font_axis)
axs[1, 0].set_ylabel('Diameter Ratio [-]', fontsize=font_axis)
axs[1, 0].set_title('Specific Power [kW/m3]', fontsize=font_title)

axs[1, 1].set_xlabel('Disk Gap [m]', fontsize=font_axis)
axs[1, 1].set_ylabel('Diameter Ratio [-]', fontsize=font_axis)
