...
output_array = np.empty((len(range(n_setpoints)) * len(range(n_setpoints)), 14))
...
for i in tqdm(range(n_setpoints)):
    ...
    for j in tqdm(range(n_setpoints)):
        ...
        new_BL.volume = tt.volume
        new_BL.d = (tt.geometry.d_main / tt.geometry.rotor.d_ratio)   # Internal Bearing Diameter
        ...
        new_BL.n = tt.rotor.rpm

        # Thermodynamic Parameters
        new_BL.m_dot = tt.rotor.m_dot_ch * tt.geometry.rotor.n_discs * tt.n_packs
        new_BL.p = rotor_array[-1, 11]
        new_BL.v = rotor_array[-1, 9]
        new_BL.A_out = np.pi * (tt.geometry.d_main / tt.geometry.rotor.d_ratio) ** 2 / 4

        new_BL.evaluate_bearing_losses()

        tt.evaluate_bearing_performances(new_BL.M_lost)
        ...
        if tt.Eta_tesla_ss < 1 and tt.Eta_tesla_ss > 0:
            ...
            output_array[j + j_0, 4] = new_BL.M_lost / (tt.work * tt.n_packs) * 100
            ...
            output_array[j + j_0, 11] = new_BL.M_lost
            output_array[j + j_0, 12] = tt.rotor.omega
            output_array[j + j_0, 13] = tt.Eta_bearings
...
axs[1, 0].set(xlabel='Tip Speed Velocity Ratio [-]', ylabel='Throat Width [m]', title='Power [kW]')
axs[1, 1].set(xlabel='Tip Speed Velocity Ratio [-]', ylabel='Throat Width [m]', title='Disc Number [-]')
...
ETA = axs[0, 0].contourf(xi, yi, res1_interp, 15)
MFR = axs[0, 1].contourf(xi, yi, res2_interp, 10)
SPEC_POWER = axs[1, 0].contourf(xi, yi, res3_interp, 15)
N_PACKS = axs[1, 1].contourf(xi, yi, res4_interp, 10)
...
res1 = (100 * (output_array[:, 10] - output_array[:, 13])).reshape(dv_perc.shape)
res2 = ((100 * output_array[:, 11] * output_array[:, 13]) / (output_array[:, 3] * output_array[:, 6])).reshape(dv_perc.shape)
