...
P_in = 93.4216833453908 * 100000  # [Pa]
P_out = 48.1791011084544 * 100000         # [Pa]
T_in_c = 77.3202882982507  # [°C]
T_in = T_in_c + 273.15  # [K]

T_tank = 43.74291548 + 273.15
...
curr_options.rotor.n_rotor = 5000
curr_options.rotor.heat_exchange = False
curr_options.rotor.tank_temp = T_tank
...
T_tank_array = np.full(rotor_array.shape[0], tt.rotor.tank_temp - 273.15)
ax.plot(rotor_array[2:, 1], (rotor_array[2:, 13] - 273.15), label="Fluid Temperature", color="Darkred")
ax.plot(rotor_array[2:, 1], T_tank_array[2:], label="Disc Temperature", color="Black")

ax.set_xlabel("Radial Position [m]", color="Black", fontsize=14)
ax.set_ylabel("Temperature [m/s]", color="Black", fontsize=14)
