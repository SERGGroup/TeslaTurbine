...
D_ratio_ref = 3  # [-]
b_ref = 0.001  # [m]
D_ratio_list = np.linspace(0.66, 1.33, n_setpoints) * D_ratio_ref
b_channel_list = np.linspace(0.3, 1.5, n_setpoints) * b_ref
b_channel, D_ratio = np.meshgrid(b_channel_list, D_ratio_list)
...
Eta = np.empty(len(b_channel_list) * len(D_ratio_list))
Power = np.empty(len(b_channel_list) * len(D_ratio_list))

Eta_arr = np.empty(len(b_channel_list) * len(D_ratio_list))
Power_arr = np.empty(len(b_channel_list) * len(D_ratio_list))
RPM_arr = np.empty(len(b_channel_list) * len(D_ratio_list))
...
for i in tqdm(range(n_setpoints)):
    ...
    for j in tqdm(range(n_setpoints)):
        ...
        curr_geometry.rotor.d_ratio = D_ratio[i, j]
...
ETA = axs[0, 0].contourf(b_channel, D_ratio, res1, 10)
MFR = axs[0, 1].contourf(b_channel, D_ratio, res2, 8)
SPEC_POWER = axs[1, 0].contourf(b_channel, D_ratio, res3, 10)
PRESSURE = axs[1, 1].contourf(b_channel, D_ratio, res4, 8)

axs[0, 0].set(xlabel='Disk Gap [m]', ylabel='D Ratio [-]', title='Efficiency [-]')
axs[0, 1].set(xlabel='Disk Gap [m]', ylabel='D Ratio [-]', title='Mass Flow Rate per Channel [kg/(s*ch)]')
axs[1, 0].set(xlabel='Disk Gap [m]', ylabel='D Ratio [-]', title='Specific Power [kW / m3]')
axs[1, 1].set(xlabel='Disk Gap [m]', ylabel='D Ratio [-]', title='Stator Outlet Pressure [Pa]')
...
plt.suptitle("Performance at D = 0.4 m, dv_perc = - 0.4")
