...
n_setpoints = 20
D_ref = 0.15
D_list = np.linspace(0.6, 1.4, n_setpoints) * D_ref
TW_ref = 0.0035  # [m]
TW_list = np.linspace(0.5,  1, n_setpoints) * TW_ref
D, TW = np.meshgrid(D_list, TW_list)
...
Eta_rotor = np.empty(len(D_list) * len(TW_list))
Power = np.empty(len(D_list) * len(TW_list))

Eta_rotor_arr = np.empty(len(D_list) * len(TW_list))
Power_arr = np.empty(len(D_list) * len(TW_list))
RPM_arr = np.empty(len(D_list) * len(TW_list))

output_array = np.empty((len(range(n_setpoints)) * len(range(n_setpoints)), 7))

Eta_max_arr = np.empty(len(D_list))
dv_perc_max_arr = np.empty(len(D_list))
tw_max_arr = np.empty(len(D_list))
...
Eta_rotor_res = Eta_rotor.reshape(D.shape)
Power_res = Power_arr.reshape(D.shape)
RPM_res = RPM_arr.reshape(D.shape)

# %%------------   PLOT RESULTS                            ----------------------------------------------------------> #

res1 = output_array[:, 1].reshape(D.shape)
res2 = output_array[:, 4].reshape(D.shape)
res3 = output_array[:, 3].reshape(D.shape)
res4 = output_array[:, 6].reshape(D.shape)

fig, axs = plt.subplots(2, 2, figsize=(12, 8), constrained_layout=True)

ETA = axs[0, 0].contourf(D, TW, res1, 15)
RPM = axs[0, 1].contourf(D, TW, res2, 8)
POWER = axs[1, 0].contourf(D, TW, res3, 10)
PR = axs[1, 1].contourf(D, TW, res4, 8)

axs[0, 0].set(xlabel='Rotor Diameter [m]', ylabel='Throat Width [m]', title='Rotor Efficiency [-]')
axs[0, 1].set(xlabel='Rotor Diameter [m]', ylabel='Throat Width [m]', title='Rotational Speed [rpm]')
axs[1, 0].set(xlabel='Rotor Diameter [m]', ylabel='Throat Width [m]', title='Power (per Channel) [W]')
axs[1, 1].set(xlabel='Rotor Diameter [m]', ylabel='Throat Width [m]', title='Pressure Ratio [-]')
