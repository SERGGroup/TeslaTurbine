...
D = 0.15        # [m]
D_list = np.linspace(0.4, 2.5, n_setpoints) * D
b_channel, D = np.meshgrid(b_channel_list, D_list)
...
Eta_rotor = np.empty(len(b_channel_list) * len(D_list))
Power = np.empty(len(b_channel_list) * len(D_list))

Eta_rotor_arr = np.empty(len(b_channel_list) * len(D_list))
Power_arr = np.empty(len(b_channel_list) * len(D_list))
RPM_arr = np.empty(len(b_channel_list) * len(D_list))

output_array = np.empty((len(range(n_setpoints)) * len(range(n_setpoints)), 8))

Eta_max_arr = np.empty(len(b_channel_list))
D_max_arr = np.empty(len(b_channel_list))
b_max_arr = np.empty(len(b_channel_list))
...
ETA = axs[0, 0].contourf(b_channel, D, res1, 15)
RPM = axs[0, 1].contourf(b_channel, D, res2, 8)
POWER = axs[1, 0].contourf(b_channel, D, res3, 10)
PR = axs[1, 1].contourf(b_channel, D, res4, 8)

# axs[0, 0].plot(dv_perc_max_arr, np_max_arr, color='Darkred', linewidth='3', linestyle='-', alpha=0.5)
axs[0, 0].set(xlabel='Disk Gap [m]', ylabel='Diameter [m]', title='Rotor Efficiency [-]')
axs[0, 1].set(xlabel='Disk Gap [m]', ylabel='Diameter [m]', title='Nozzle Outlet Velocity [m/s]')
axs[1, 0].set(xlabel='Disk Gap [m]', ylabel='Diameter [m]', title='Power [W]')
axs[1, 1].set(xlabel='Disk Gap [m]', ylabel='Diameter [m]', title='Static-to-Static Pressure Variation [bar]')
...
plt.suptitle("Performance at P_in = 80 bar, TW = 3.5 mm, Tip Ratio = 1, n_packs = 60")
