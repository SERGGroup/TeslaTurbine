...
P_in = 22000000   # [Pa]
P_out = 21000000
T_in = np.linspace(398.15, 448.15, 50) # K
...
output_array = np.empty((len(T_in), 8))
...
# %%------------             IMPORT EES RESULTS                ------------------------------------------------------> #
py_df = pd.DataFrame(output_array, columns=['T_00', 'Eta_Tesla_ss', 'Work', 'Power', 'RPM', 'm_dot', "P_out", "T_out"])
RESULT_FOLDER = os.path.join(CALCULATION_FOLDER, "single_phase_validation", "results")
excel_file = os.path.join(RESULT_FOLDER, "T_00_variation.xlsx")
